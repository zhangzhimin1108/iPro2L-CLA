...
seq1 = open_fa('D:/PycharmProjects/pythonProject/iPromoter-CLA/data/promoter.txt')
seq2 = open_fa('D:/PycharmProjects/pythonProject/iPromoter-CLA/data/non-promoter.txt')
seq1_onehot = onehot(seq1)
seq2_onehot = onehot(seq2)
seq1_onehot=np.array(seq1_onehot)
seq2_onehot=np.array(seq2_onehot)
X=np.concatenate((seq1_onehot,seq2_onehot),axis=0)
...
for i,(train, test) in enumerate(kfold.split(X, Y)):
    ...
    def get_model():
        ...
        x = Conv1D(32,7 , activation='relu', padding='valid',kernel_initializer='he_normal')(inputs)
        x = MaxPooling1D(pool_size=2, strides=2, padding='same')(x)
        x = BatchNormalization()(x)
        x = Dropout(0.5)(x)
        x = Conv1D(32, 5, activation='relu', padding='valid',kernel_initializer='he_normal')(x)
        x = MaxPooling1D(pool_size=2, strides=2, padding='same')(x)
        x = BatchNormalization()(x)
        x = Dropout(0.5)(x)
    ...
    predict_acc_1 = model1.predict(X[test])
    predict_acc_2 = []
    ...
    ###########################
    predict_test_y = model1.predict(X[test])
    ...
    score,acc = model1.evaluate(X[test], Y[test])
